...
#         returnMat.append([float(line[0]),float(line[1]),float(line[2])])
...
def autoNorm(data):
    ...
    m = data.shape[0]
    normDataSet = data - np.tile(minVals,(m,1))        #tile函数将变量内容复制成输入矩阵同样大小的矩阵
...
# def find_NN(tree, x):
#     """
#     最近邻搜索，kd_tree为根据训练集构造的kd树
#     x为要预测的实例
#     """
#     NN_node = None
# #     if tree.left or tree.right:       #递归查找最近的叶节点
#     if tree.left and tree.value[tree.dimension] < x[tree.dimension]:
#         NN_node = find_NN(tree.left, x)
#         print(NN_node)
#         #使用L2距离计算
#         #检查父节点
#         if (tree.value[0]-x[0])**2+(tree.value[1]-x[1])**2+(tree.value[2]-x[2])**2 <\
#            (NN_node.value[0]-x[0])**2+(NN_node.value[1]-x[1])**2+(NN_node.value[2]-x[2])**2:
#             NN_node = tree
#         #检查兄弟结点
#         if tree.right and (tree.right.value[0]-x[0])**2+(tree.right.value[1]-x[1])**2+(tree.right.value[2]-x[2])**2 <\
#            (NN_node.value[0]-x[0])**2+(NN_node.value[1]-x[1])**2+(NN_node.value[2]-x[2])**2:
#             NN_node = tree.right
#     elif tree.right and tree.value[tree.dimension] > x[tree.dimension]:
#         NN_node=find_NN(tree.right, x)
#         print(NN_node)
#         if (tree.value[0]-x[0])**2+(tree.value[1]-x[1])**2+(tree.value[2]-x[2])**2 <\
#            (NN_node.value[0]-x[0])**2+(NN_node.value[1]-x[1])**2+(NN_node.value[2]-x[2])**2:
#             NN_node = tree
#         #检查兄弟结点
#         if tree.left and (tree.left.value[0]-x[0])**2+(tree.left.value[1]-x[1])**2+(tree.left.value[2]-x[2])**2 <\
#            (NN_node.value[0]-x[0])**2+(NN_node.value[1]-x[1])**2+(NN_node.value[2]-x[2])**2:
#             NN_node = tree.left
#             
#         return NN_node
